...
class AuthProvider extends ChangeNotifier {
  ...
  updateUserImage(File fil) {
    userImage = fil;
    notifyListeners();
  }

  updateDispose(bool val) {
    isDispose = val;
    print("values of is dispose ${isDispose}");
    print("values of is dispose ${val}");
  }

  String jntMgtphncod = "44";
  String? jntMgtcntrycode;

  setJntMgtCountry(String jMPhCode, String jMCnCde) {
    jntMgtphncod = jMPhCode;
    jntMgtcntrycode = jMCnCde;
    notifyListeners();
  }

  setCountry(String cntryCodee, String phnCodee, String flagggg) {
    phncode = phnCodee;
    cntrycode = cntryCodee;
    cntryflag = flagggg;
    print("object updatinggggg");
    // print("update countryy codeeeee${userModel?.shortCode??""}");
    //HiveHandler.updatePhoneCode(phncode??"");
    //HiveHandler.updateCountryCode(cntryCodee);

    print("update countryy codeeeee${userModel?.shortCode ?? ""}");

    print("auth provider set phonecode${phncode}");
    notifyListeners();
  }

  String? newFlag;

  changeFlag(String updatedCode) {
    newFlag = userModel?.shortCode?.toUpperCase().replaceAllMapped(
            RegExp(r'[A-Z]'),
            (match) =>
  ...
  String phncode1 = "44";
  String cntrycode1 = "GB";
  String cntryflag1 = "🇬🇧";

  settttCountreee1(String p1, p2, p3) {
    phncode1 = p1;
    cntrycode1 = p2;
    cntryflag1 = p3;
  }
...
int usrNameEmpty=0;
  ...
  PetTypeModl? selectedUserGender;

  List<PetTypeModl> userGenderList = [
    PetTypeModl(title: tr(LocaleKeys.additionText_maleE), typeId: "1"),
    PetTypeModl(title: tr(LocaleKeys.additionText_femaleE), typeId: "2"),
  ];

  onselectUserGender(PetTypeModl val) {
    Future.delayed(Duration.zero, () async {
      selectedUserGender = val;
      print("recuring TimereTime===>> ${selectedUserGender}");
      notifyListeners();
    });
  }
  ...
  static callUpdateDeviceFunction(BuildContext context) async {
    Map deviceInfoBody = {};
    PackageInfo packageInfo = await PackageInfo.fromPlatform();
    DeviceInfoPlugin deviceInfoPlugin = DeviceInfoPlugin();
    print("packageInfo.version app version ${packageInfo.version}");
    token = await HiveHandler.getDeviceToken();
    ...
    print("device token body $deviceInfoBody");
    await AppApi.deviceapi(deviceInfoBody).then((value) {
  ...
  googleLogin(BuildContext context) async {
    updateLoader(true);
    var user = await FirebaseHandler.googleLogin().catchError((e) async {
      print("issue in google login $e");
      updateLoader(false);
    });
    if (user != null) {
      ...
      if(user.displayName=="" || user.displayName==null){
        print("user===>>${user.email.split('@')[0]}");
       usrName=user.email.split('@')[0];
        usrNameEmpty=1;
      }
      else{
      ...
      commonSocialLoginApi(user, loginType.google, context);
    } else {
      await loaderUpdate(false);
  ...
  facebookLogin(BuildContext context) async {
    ...
    var user = await FirebaseHandler.facebookLogin().catchError((e) {
      updateLoader(false);
    });
    ...
    if (user != null) {
      ...
      commonSocialLoginApi(user, loginType.facebook, context);
    } else {
      updateLoader(false);
  ...
  appleLogin(BuildContext context) async {
    ...
    print("inside apple login");
    await updateLoader(true);
    var user = await FirebaseHandler.signInWithApple().catchError((e) {
      updateLoader(false);
    });
    ...
    if (user != null) {
      ...
      commonSocialLoginApi(user, loginType.apple, context);
    } else {
      await updateLoader(false);
  ...
  commonSocialLoginApi(
      UserSocial user, loginType value, BuildContext context) async {
    print("check apple EMAIL  ${user.email}");
    print(
        "hive image profile ${HiveHandler.getUser()?.profileImage.toString()}");
    Map _body = {
      "email": user.email,
      "type": _getSocialLoginType(value),
    };

    if (value == loginType.google) {
      print("check google login body  ${_body}");
      _body.putIfAbsent('tokenId', () => user.uid);
      _body.putIfAbsent('token', () => user.token);
    }

    if (value == loginType.apple) {
      _body.putIfAbsent('tokenId', () => user.uid);
      _body.putIfAbsent('token', () => user.token);
    }
    ...
    print("check apple login body  ${_body}");

    var res = await api.socialLoingApi(_body).catchError((e) {
      print(">>>>>> ERROR IN SOCIAL LOGIN API $e<<<<<<<<<<");
      CoolAlert.show(
    ...
    if (res.status == Status.success) {
      print("check isNewLogin val  ${isNewLogin}");

      isNewLogin = 1;

      print("check isNewLogin after new val set ${isNewLogin}");

      print("USER PASSSWORD ${res.password}");
      var _userName = res.name ?? "";
      if (_userName.isEmpty) {
        HiveHandler.addUser(res);
        Navigator.pushNamed(context, AppScreen.signUpSuccess);
      } else {
        HiveHandler.addUser(res);
      ...
      CoolAlert.show(
          context: context, type: CoolAlertType.error, text: res.message);
    }
  }

  normalLogin(Map body, BuildContext context) async {
    print("on tap loader status $loader");
    if (loader) {
      return;
    }
    ...
    if (!loader) {
      ...
      await api.signupApi(body).then((res) async {
        await updateLoader(false);
        if (res.status == Status.success) {
          await CoolAlert.show(
              context: context, type: CoolAlertType.success, text: res.message);
          Navigator.pushNamed(context, AppScreen.signIn);
        } else {
          CoolAlert.show(
              context: context, type: CoolAlertType.warning, text: res.message);
        }
      }).onError((error, stackTrace) => updateLoader(false));
    }
  }

  updateNameApi(Map<String, dynamic> body, BuildContext context) async {
    updateLoader(true);
    print("unknown body parameter>>>>>>>>${body}");
    await api.callupdateNameApi(body).then((value) async {
      updateLoader(false);
      if (value.status == Status.success) {
        //   var newName = value.data["name"];
        var Name = value.data["user"];
        var newName = Name["name"];
        print("object name>>>>>>${Name}");
        print("object new name >>>>>>${newName}");
        if (newName.isNotEmpty) {
          HiveHandler.updateUserName(newName);
          CoolAlert.show(
              context: context,
  ...
  callLoginApi(Map<String, dynamic> body, BuildContext context) async {
    updateLoader(true);
    isNewLogin = 0;
    var res = await api.loginCalling(body).catchError((e) {
      print(">>>>>> ERROR IN  LOGIN API $e<<<<<<<<<<");
      updateLoader(false);
    });
    await updateLoader(false);
    print("check is loader is still true=====");
    Future.delayed(Duration(milliseconds: 20), () {
      if (chkLoader == true) {
        print("loader is still true=====");
      }
    });
    ...
    if (res.status == Status.success) {
      ...
      if (_userName.isEmpty) {
        ...
        FirebaseHandler.firebaseTempLogin();
        HiveHandler.addUser(res);
        print("updating ph codeeee");
        //     HiveHandler.updatePhoneCode(userModel?.phoneCode??"");
        //   HiveHandler.updateCountryCode(userModel?.shortCode??"");
        print(
            "hive image profile ${HiveHandler.getUser()?.profileImage.toString()}");
        Navigator.pushNamedAndRemoveUntil(
            context, AppScreen.dashboard, (route) => false);
  ...
  _getSocialLoginType(loginType value) {
    switch (value) {
  ...
  updateForgotPassword(
      Map body, BuildContext ctx, String emailtobesending) async {
    updateLoader(true);
    await api.forgetPasswordApi(body).then((value) async {
      updateLoader(false);
      if (value.status == Status.success) {
        await CoolAlert.show(
          context: ctx,
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: ((context) => CreatePassword(
                      emailsend: emailtobesending,
                    ))));
        print("===========Email sent on your email id==========");
      } else {
        CoolAlert.show(
          context: ctx,
  ...
  callChangePassword(Map<String, dynamic> body, BuildContext context) async {
    print("bodyyyy $body");
    updateLoader(true);
    await api.changePasswordApi(body).then((res) async {
      updateLoader(false);
      if (res.status == Status.success) {
        await CoolAlert.show(
            context: context, type: CoolAlertType.success, text: res.message);
  ...
  resetPasswordApi(Map<String, dynamic> body, BuildContext context) async {
    updateLoader(true);
    await api.callresetPassword(body).then((res) async {
      updateLoader(false);
      if (res.status == Status.success) {
        await CoolAlert.show(
            context: context, type: CoolAlertType.success, text: res.message);
  ...
  updateUserProfileApi({
    ...
    required String nameee,
    required String lastNameee,
    required String countryy,
    required String cityy,
    required String onrAddress,
    String? phonenum,
    String? phoneCodee,
    String? cntreShortCodee,
  }) async {
    String urlOfImg = "";
    print("printingggggg");
    if (userImage != null) {
      List images = [
        {"key": "image", "image": userImage}
      ];
      updateLoader(true);
      ...
      String pic = userModel?.profileImage ?? "";
      if (pic.isEmpty) {
        urlOfImg = "";
      }
    }
    ...
    Map<String, dynamic> bodyy = {
      ...
      "country": countryy,
      "city": cityy,
      "profileImage": urlOfImg,
      "gender": selectedUserGender?.typeId ?? "",
      "mobileNumber": phonenum,
      "phoneCode": phoneCodee,
      "shortCode": cntreShortCodee,
      "address": onrAddress
    };

    print("REQUESTTT ${bodyy}");
    print("urlOfImg>>>>>>><<<<<${urlOfImg}");
    updateLoader(true);
    var res = await api.editProfileP2Api(bodyy);
    // var res = await api.editUserProfileApi(bodyy);
    ...
    if (res.status == Status.success) {
      ...
      print("check country before hive set=====>>${res.country}");
      print("check country phone code before hive set=====>>${res.phoneCode}");

      HiveHandler.addUser(res);
      print("updating phone codeeeeee.......");

      UserModel? ussr = HiveHandler.getUser();
      print("user country check from hive=====>${ussr?.countryName}");
      print("user country code from hive=====>${ussr?.phoneCode}");

      HiveHandler.updatePhoneCode(phoneCodee ?? "44");

      print("updating phone number.......");
      HiveHandler.updatePhNumber(phonenum!);

      // if (nameee.isNotEmpty) {
      //   print("going to hive handler...");
      // }
      // print("going to hive for name...");
      HiveHandler.updateUserName(nameee);

      print("updating countryyy codeeee.......");
      HiveHandler.updateCountryCode(cntreShortCodee ?? "GB");
  ...
  deleteAccountApi(BuildContext context) async {
    updateLoader(true);
    Map body = {};
    await api.deleteUserApi(body).then((value) async {
      updateLoader(false);
      if (value.status == Status.success) {
  ...
  logoutApiCall(BuildContext context) async {
    updateLoader(true);
    Map body = {};
    await api.logoutApi(body).then((value) async {
      updateLoader(false);
      if (value.status == Status.success) {
        HiveHandler.clearUser();
        // HiveHandler.updateLocPopup(true);
        // await CoolAlert.show(context: context, type: CoolAlertType.success, text: value.message);
        Navigator.pop(context);
        Navigator.pushNamedAndRemoveUntil(
            context, AppScreen.signIn, (r) => false);
      }
  ...
  String phncodeOnr = "GB";
  String cntrycodeOnr = "44";
  String cntryflagOnr = "🇬🇧";
  ...
  setMobile(String onr2) {
    cntrycodeOnr = onr2;

    notifyListeners();

    print("contre code for api set=${cntrycodeOnr}");
  }

  setLogoutFlag() {
    cntryflag = "🇬🇧";
    print(" value of auth flagggg${cntryflag}");
  }
  ...
  String getContryFlag(String str) {
    print("code==>>${str}");
    str = "+" + str;
    print("str ater concat${str}");
    final List<Map<String, dynamic>> countryCodes = [
      {
        "e164_cc": "93",
        "iso2_cc": "AF",
    ...
    // setOnrProfileFlag(countryCodeToEmoji(mapVal['iso2_cc']));
  }
  ...
  setOnrProfileFlag(String flg) {
    onrFlag = flg;
    print("AFTER FINAL VALUE SET flag==>>${onrFlag}");
    Future.delayed(Duration.zero, () async {
      notifyListeners();
    });
  }

  ownerCntryCodeToEmoji(String countryCode) {
    print("contry code${countryCode}");
